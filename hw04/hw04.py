...
def has_path(t, phrase):
    ...
    def all_words(t, s):
        if is_leaf(t):
            lst.append(s + label(t))
        else:
            for b in branches(t):
                all_words(b, s + label(t))

    all_words(t, "")
    return phrase in lst
...
def div_interval(x, y):
    ...
    assert not (min(lower_bound(reciprocal_y), upper_bound(reciprocal_y)) < 0 and max(
        lower_bound(reciprocal_y), upper_bound(reciprocal_y)) > 0)
